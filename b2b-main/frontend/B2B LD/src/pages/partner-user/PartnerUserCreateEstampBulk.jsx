...
const PartnerUserCreateEstampBulk = () => {
  ...
  // (see PartnerUserEstampBulkOrderDetails.jsx and create_bulk_estamp_order):
  ...
  // See GET /partner-user/services/{name}/charges.
  ...
  useEffect(() => {
    ...
    apiRequest("/api/partner-user/profile")
      .then((profile) => {
  ...
  useEffect(() => {
    ...
    apiRequest(`/api/partner-user/article-codes?state_id=${stampStateId}`)
  ...
  // requirements exactly (see backend/app/routes/partner_user.py) so nothing
  // that passes here should ever bounce off the API's own validation.
  const validateFields = () => {
    ...
    if (!partnerAddressComplete) {
      errors.partnerAddress = "Your organization's registered address is incomplete — contact Super Admin to complete your profile before placing an eStamp Bulk order.";
    }
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const order = await apiRequest("/api/partner-user/orders/estamp-bulk", {
        method: "POST",
        body: JSON.stringify({
          customer_name: null,
          customer_email: null,
      ...
      navigate(`/user/orders/estamp-bulk/${order.id}`);
    } catch (err) {
  ...
  return (
    <div>
      ...
      <div className="rounded-lg p-6 max-w-3xl" style={{ background: theme.card, border: `1px solid ${theme.border}`, boxShadow: "0 1px 2px rgba(15,23,42,0.04)" }}>
        ...
        <SectionLabel>Stamp State</SectionLabel>
                      ...
                      <div>
                        ...
                        {profileLoading ? (
                          ...
                          <div className="flex items-start gap-1.5 mt-1 px-2 py-1.5 rounded text-xs font-medium" style={{ background: theme.dangerSoft, color: theme.danger }}>
                            ...
                            <span>Registered address incomplete — contact Super Admin.</span>
                          </div>
                        )}
                      </div>
                    </div>
...
export default PartnerUserCreateEstampBulk;
