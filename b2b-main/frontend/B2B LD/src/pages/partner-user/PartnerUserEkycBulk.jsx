...
const PartnerUserEkycBulk = () => {
  ...
  const loadRecords = (overrides = {}) => {
    ...
    apiRequest(`/api/partner-user/ekyc-bulk/records?${query}`)
      .then((data) => {
  ...
  useEffect(() => {
    apiRequest("/api/partner-user/ekyc-bulk/batches?page=1&page_size=1")
      .then((data) => {
  ...
  useEffect(() => {
    ...
    apiRequest(`/api/partner-user/ekyc-bulk/batches?${query}`)
      .then((data) => {
  ...
  const handleDownloadTemplate = async () => {
    ...
    setImportError("");
    try {
      await downloadFile("/api/partner-user/ekyc-bulk/template", "ekyc_bulk_template.xlsx");
    } catch (err) {
  ...
  const handleFileSelected = async (e) => {
    ...
    try {
      ...
      const result = await apiUpload("/api/partner-user/ekyc-bulk/import", formData);
  ...
  const rowAction = (record) => {
    if (!record.order_id) {
      return (
        <button
          onClick={() =>
            navigate("/user/orders/create/eKYC", {
              state: {
                prefill: {
    ...
    return (
      <button
        onClick={() => navigate(`/user/orders/${record.order_id}`)}
...
export default PartnerUserEkycBulk;
