...
const orderDetailPath = (invoice) => {
  if (invoice.service_name === "eStamp Bulk") return `/partner/orders/estamp-bulk/${invoice.order_id}`;
  if (invoice.service_name === "Manual eStamp") return `/partner/orders/manual-estamp/${invoice.order_id}`;
  return `/partner/orders/${invoice.order_id}`;
};
...
const PartnerInvoices = () => {
  ...
  useEffect(() => {
    apiRequest("/api/partner/invoices")
      .then(setInvoices)
  ...
  const handleDownload = async (invoice) => {
    setDownloadError("");
    try {
      await downloadFile(`/api/partner/invoices/${invoice.id}/pdf`, `${invoice.invoice_number.replace(/\//g, "-")}.pdf`);
    } catch (err) {
  ...
  return (
    <div>
      ...
      {viewingInvoiceId && (
        <InvoiceViewModal
          fetchUrl={`/api/partner/invoices/${viewingInvoiceId}`}
          downloadUrl={`/api/partner/invoices/${viewingInvoiceId}/pdf`}
