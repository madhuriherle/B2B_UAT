...
const PartnerCreateManualEstamp = () => {
  ...
  // per-order question. See GET /api/partner/services/{name}/charges.
  useEffect(() => {
  ...
  useEffect(() => {
    apiRequest("/api/partner/services")
      .then((res) => {
        const svc = (res || []).find((s) => s.service_name === SERVICE_NAME);
  ...
  // Bulk uses — see /api/partner/stamp-denominations), scoped to
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // empty string and rejects it (same fix as PartnerCreateOrder.jsx).
      ...
      const order = await apiUpload("/api/partner/orders/manual-estamp", formData);
      navigate(`/partner/orders/manual-estamp/${order.id}`);
    } catch (err) {
...
export default PartnerCreateManualEstamp;
