...
const PartnerCreateOrder = () => {
  ...
  // PartnerEkycBulk.jsx's "Initiate eKYC" navigate call) — threaded through
  // to submitOrder below so the created order gets linked back to that CSV
  // row (see partner._create_order's bulk_ekyc_record_id param). Stays null
  // for every other entry point. Unlike the User Portal's version, there's
  // no dedicated /orders/create/eKYC route here (this screen is a single
  // /orders/create page with an in-page service dropdown), so the handoff
  // is detected from router state alone, not a URL param.
  ...
  useEffect(() => {
    ...
    // PartnerEkycBulk.jsx) — there's no dedicated /orders/create/eKYC route
    // on this portal to key it off of the way the User Portal does, so this
    // just checks whether a prefill was actually handed off, and treats it
    // as an eKYC prefill unconditionally (Bulk eKYC only ever produces
    // eKYC orders).
    const prefill = location.state?.prefill || null;
    setForm({
      ...emptyForm,
      service_name: prefill ? "eKYC" : preselectedService,
  ...
  // Every service enabled for the org as a whole — same {services,
  // documents} shape the User Portal's own form normalizes to.
  useEffect(() => {
    setServicesLoading(true);
    const request = Promise.all([
      apiRequest("/api/partner/services"),
      apiRequest("/api/partner/services/documents"),
    ]).then(([serviceRows, documentRows]) => ({
      services: serviceRows.filter((s) => s.is_active).map((s) => ({ service_name: s.service_name, price: s.price, assigned: true })),
      documents: documentRows.map((d) => ({ ...d, assigned: true })),
    }));
    ...
    request
      .then((data) => {
        ...
        // as a selectable option, same as the User Portal's own form.
  ...
  useEffect(() => {
    ...
    apiRequest(`/api/partner/services/${encodeURIComponent(form.service_name)}/charges`)
  ...
  const submitOrder = async (action) => {
    ...
    try {
      ...
      const order = await apiUpload("/api/partner/orders", formData);
      ...
      if (isEsign && action === "submit") {
        try {
          const esign = await apiRequest(`/api/partner/orders/${order.id}/esign/initiate`, {
            method: "POST",
        ...
        try {
          await apiRequest(`/api/partner/orders/${order.id}/ekyc/verify`, {
            method: "POST",
        ...
        try {
          const fullOrder = await apiRequest(`/api/partner/orders/${order.id}`);
          setResult(fullOrder);
        } catch {
          setResult(order);
        }
        ...
        try {
          const stamp = await apiRequest(`/api/partner/orders/${order.id}/stamp/initiate`, {
            method: "POST",
          ...
          if (wantEsignAfterStamp) {
            if (stamp.status === "completed") {
              try {
                esign = await apiRequest(`/api/partner/orders/${order.id}/esign/initiate`, {
                  method: "POST",
        ...
        try {
          const stampOtf = await apiRequest(`/api/partner/orders/${order.id}/stamp-otf/initiate`, {
            method: "POST",
  ...
  const handleDownloadStampedDocument = async () => {
    ...
    try {
      await downloadFile(`/api/partner/orders/${result.id}/document/stamped-preview`, `${result.order_no}-stamped.pdf`);
    } catch (err) {
  ...
  const handleDownloadOtfStampedDocument = async () => {
    ...
    try {
      await downloadFile(`/api/partner/orders/${result.id}/document/stamp-otf-preview`, `${result.order_no}-stamped.pdf`);
    } catch (err) {
  ...
  const refreshResult = async () => {
    if (!result?.id) return;
    try {
      const fullOrder = await apiRequest(`/api/partner/orders/${result.id}`);
      setResult(fullOrder);
    } catch {
  ...
  const handleRetryEkycVerification = async () => {
    ...
    setError("");
    try {
      await apiRequest(`/api/partner/orders/${result.id}/ekyc/verify`, {
        method: "POST",
        body: JSON.stringify({ verification: verifyDocument }),
      });
      await refreshResult();
  ...
  const handleDigilockerVerify = async () => {
    ...
    setDigilockerError("");
    try {
      const digilocker = await apiRequest(`/api/partner/orders/${result.id}/digilocker/verify`, { method: "POST" });
      await refreshResult();
  ...
  const handleFetchAadhaarDetails = async () => {
    ...
    setDigilockerError("");
    try {
      await apiRequest(`/api/partner/orders/${result.id}/digilocker/fetch-aadhaar`, { method: "POST" });
      await refreshResult();
    } catch (err) {
      setDigilockerError(err.message);
    } finally {
    ...
    return (
      <div>
        <div className="mb-6 pb-4 border-b" style={{ borderColor: theme.border }}>
          <button
            type="button"
            onClick={() => navigate("/partner/orders/create/eKYC-bulk")}
          ...
          {ekycOutcome ? (
            ...
            >
              ...
              <div className="mt-4 flex items-center justify-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => navigate(`/partner/orders/${result.id}`)}
                  className="px-5 py-2.5 rounded text-sm font-semibold text-white transition-opacity hover:opacity-90"
