...
import { STATUS_STYLES, formatDate } from "./orderShared";
...
const PartnerManualEstampOrderDetails = () => {
      ...
      <div className="flex justify-between items-start mb-6 pb-4 border-b" style={{ borderColor: theme.border }}>
        ...
        <div className="flex items-center gap-3">
          {order.status === "Completed" && manual?.invoice_number && (
            <button
              onClick={() => download(`/api/partner/orders/${orderId}/invoice/pdf`, `${order.order_no}-invoice.pdf`)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6">
        ...
        <div className="space-y-4">
          <section className="rounded-lg p-6 space-y-3" style={card}>
            ...
            {order.document_path && (
              <DocumentRow label="Original Document" onDownload={() => download(`/api/partner/orders/${orderId}/document`, `${order.order_no}-original.pdf`)} />
            )}

            {manual?.stamped_document_path ? (
              <DocumentRow label="Stamped Document" onDownload={() => download(`/api/partner/orders/${orderId}/document/manual-stamped-preview`, `${order.order_no}-stamped.pdf`)} />
            ) : (
            ...
            {manual?.esign_required && (
              esign?.signed_file_path ? (
                <DocumentRow label="Final Signed Document" onDownload={() => download(`/api/partner/orders/${orderId}/document/signed-preview`, `${order.order_no}-signed.pdf`)} />
              ) : (
                <div className="p-3 rounded text-xs" style={{ background: theme.bg, color: theme.slate, border: `1px solid ${theme.border}` }}>
...
export default PartnerManualEstampOrderDetails;
